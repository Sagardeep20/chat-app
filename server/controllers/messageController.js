...
export const sendMessage = async (req, res) => {
    try {
        ...
        const recvSet = userSocketMap[recvKey];
        if(recvSet){
            for (const sid of recvSet) {
                io.to(sid).emit("newMessage", newMessage);
            }
        }

        //Emit to sender's other sockets as well (so multiple tabs update)
        const senderKey = String(senderId);
        const senderSet = userSocketMap[senderKey];
        if(senderSet){
